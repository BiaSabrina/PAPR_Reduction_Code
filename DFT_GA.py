...
#%% Cálculo da IFFT e FFT:
    
def FFT(symbol):
    OFDM_time = np.fft.fft(symbol).astype('complex64')    
    return OFDM_time
    
def IFFT(fft_):
    OFDM_time = np.sqrt(N-Np) * np.fft.ifft(fft_).astype('complex64')    
    return OFDM_time
...
DFT = FFT(mapper_final)
OFDM_time_DFT = IFFT(DFT)
PAPR_dB_DFT = PAPR(OFDM_time_DFT)
CCDF_DFT = CCDF(PAPR_dB_DFT) 
...
ax.semilogy(np.arange(min(PAPR_dB_DFT), max(PAPR_dB_DFT), 0.00001), CCDF_DFT, '--', color='C1', label="DFT Signal")
ax.semilogy(np.arange(min(PAPR_final), max(PAPR_final), 0.00001), CCDF_red, 'o', color='C2', label="GA Signal")
...
class UncodedSystemAWGN_DFT(Model): 
    ...
    def __call__(self, batch_size, ebno_db):
        ...
        global OFDM_time_DFT     
        global bits_final
        
        no = sn.utils.ebnodb2no(ebno_db,
                                num_bits_per_symbol=self.num_bits_per_symbol,
                                coderate=1.0)       
        self.OFDM_RX_FD_ = OFDM_time_DFT
        y__ = self.awgn_channel([self.OFDM_RX_FD_, no]) # no = potência do ruído       
        y__= FFT(y__)
        y__= IFFT(y__)
        OFDM_demod__ = y__
        print(OFDM_demod__.size)
        llr_DFT = self.demapper([OFDM_demod__,no])
        print(llr_DFT)
        return bits_final, llr_DFT
...
model_uncoded_awgn_DFT = UncodedSystemAWGN_DFT(num_bits_per_symbol=NUM_BITS_PER_SYMBOL, block_length=BLOCK_LENGTH, Subcarriers=N)
...
ax.plot(ebno_dbs, BER_SIM_DFT, '->', markersize=5, color='C2', label='DFT Signal')

# Plot BER for the GA signal with crosses
ax.plot(ebno_dbs, BER_SIM_GA, 'x', markersize=5, color='C3', label='GA Signal')
