...
Ntx=50# Número de símbolos OFDM que serão gerados.
...
def pilot_value_change(batch, Np, pilots):
    ...
    return matrix
'''
def pilot_value_change_NN(batch, Np, pilots):
    # Certifica-se de que pilots seja tratado como uma matriz 2D
    pilots = np.atleast_2d(pilots)
    matrix = np.zeros((batch, Np), dtype=complex)

    for i in range(batch):
        for j in range(Np): 
            matrix[i, j] = pilots[i % pilots.shape[0], j]  # Ajusta para múltiplas linhas, se necessário
    return matrix
'''    
...
def symbol(x_, pilots):
    ...
    return symbol
'''
def symbol_NN(x_, pilots):
    allCarriers = create_matrix(int(np.size(x_) / (N - Np)), N)
    pilotCarriers = create_pilot(int(np.size(x_) / (N - Np)), Np)
    dataCarriers = np.delete(allCarriers, pilotCarriers, axis=1)     
    symbol = np.zeros((int(np.size(x_) / (N - Np)), N), dtype=complex)
    pilots_values = pilot_value_change_NN(int(np.size(x_) / (N - Np)), Np, pilots)
    symbol[:, pilotCarriers] = pilots_values
    symbol[np.arange(int(np.size(x_) / (N - Np)))[:, None], dataCarriers] = x_
    return symbol
'''
...
symbol_NN = np.zeros((Ntx,N),dtype=complex)
...
def IFFT_training(tensor):
    ifft_ = np.sqrt(N)*tf.signal.ifft(tensor).astype('complex64')  
    return ifft_
'''
def pilot_value_change_training(pil):
    pil = tf.complex(pil[:,0:Np], pil[:,Np:2*Np])
...
def symbol_training(x_, pil):    
    ...
    # Initialize symbol_ with zeros
    symbol_ = np.zeros((batch, N), dtype='complex64')
    symbol_[:, data_Carriers] = x_

    # Ensure pilot_Carriers is in the right shape
    pilot_Carriers = pilot_Carriers.reshape(-1, 2)

    # Generate pilots_values_ for training
    pilots_values_ = pilot_value_change_training(pil)

    # Convert symbol_ to a TensorFlow tensor
    symbol_tensor = tf.constant(symbol_, dtype=tf.complex64)

    # Update symbol_ tensor with pilots_values_
    updated_symbol_tensor = tf.tensor_scatter_nd_update(symbol_tensor, pilot_Carriers, pilots_values_)
    
    return updated_symbol_tensor.numpy() 
...
model.add(Dense(2*N, kernel_initializer=initializer))


optimizer = Adam(learning_rate=0.0001)
lamba = 0.02

model.compile(loss=papr, optimizer=optimizer)


# Treinando o modelo
st = time.time()
history = model.fit(dataset_original, dataset_original, epochs= 1000, verbose=2, shuffle=True)
et = time.time()
...
for ijx in range(Ntx):
    symbol_NN[ijx] = symbol(mod[ijx], RED[ijx])
    
IFFT_pilots_NN = IFFT(symbol_NN)
...
for i in range(NN):
   ...
   P1 = np.concatenate([ofdm_symbol[i, 0:16], np.zeros(48)])
   P2 = np.concatenate([np.zeros(16), ofdm_symbol[i, 16:32], np.zeros(32)])
   P3 = np.concatenate([np.zeros(32), ofdm_symbol[i, 32:48], np.zeros(16)])
   P4 = np.concatenate([np.zeros(48), ofdm_symbol[i, 48:64]])
...
class UncodedSystemAWGN_PTS(Model): 
    ...
    def __call__(self, batch_size, ebno_db):
        ...
        ofdm_symbol_reconstructed = np.concatenate((F1[:, 0:16], F2[:, 16:32], F3[:, 32:48], F4[:, 48:64]), axis=1)  
