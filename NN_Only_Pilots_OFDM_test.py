...
dataset_otimizado = np.c_[np.real(successful_pilots), np.imag(successful_pilots)]

# Callbacks para monitorar m_t e v_t
class AdamTrackingCallback(Callback):
    def __init__(self, model, dataset_original, dataset_otimizado):
        self.model = model
        self.dataset_original = dataset_original
        self.dataset_otimizado = dataset_otimizado
        self.mt_values = []
        self.vt_values = []
        self.times = []
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        with tf.GradientTape() as tape:
            predictions = self.model(self.dataset_original, training=True)
            loss = tf.reduce_mean(tf.keras.losses.mean_squared_error(self.dataset_otimizado, predictions))
        gradients = tape.gradient(loss, self.model.trainable_weights)
        optimizer = self.model.optimizer
        m_t = [optimizer.beta_1 * g + (1 - optimizer.beta_1) * g for g in gradients]
        v_t = [optimizer.beta_2 * g ** 2 + (1 - optimizer.beta_2) * g ** 2 for g in gradients]
        self.mt_values.append(np.mean([np.mean(mt.numpy()) for mt in m_t]))
        self.vt_values.append(np.mean([np.mean(vt.numpy()) for vt in v_t]))
...
adam_tracker = AdamTrackingCallback(model, dataset_original, dataset_otimizado)

# Treinamento
st = time.time()
history = model.fit(dataset_original, dataset_otimizado, epochs=500,
                    validation_data=[dataset_original, dataset_otimizado], verbose=2, shuffle=True,
                    callbacks=[adam_tracker])
et = time.time()
...
RED = RED[:,0:Np] + 1j*RED[:,Np:Np*2]

symbol_Ori_NN = np.zeros((Ntx,N),dtype=complex)
                                  
for ij in range(Ntx):
    symbol_Ori_NN[ij] = symbol(mapper_final_no[ij], RED[ij])
    
IFFT_pilots_NN = IFFT(symbol_Ori_NN)
...
ax1.plot(adam_tracker.mt_values, label='m_t (First Moment Estimate)', color='blue')
ax1.plot(adam_tracker.vt_values, label='v_t (Second Moment Estimate)', color='red')
ax1.set_ylabel('Values', fontsize=14)
ax1.grid(True, linestyle='--', alpha=0.5)
ax1.legend(fontsize=12)
ax1.set_title('Adam Optimizer - m_t and v_t over Epochs', fontsize=16)

ax2.plot(adam_tracker.times, label='m_t (First Moment Estimate)', color='blue')
ax2.plot(adam_tracker.times, label='v_t (Second Moment Estimate)', color='red')
ax2.set_xlabel('Epochs', fontsize=14)
ax2.set_ylabel('Time (s)', fontsize=14)
ax2.grid(True, linestyle='--', alpha=0.5)
ax2.legend(fontsize=12)
ax2.set_title('Adam Optimizer - m_t and v_t over Time', fontsize=16)
